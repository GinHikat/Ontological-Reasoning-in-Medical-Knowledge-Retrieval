...
class EntityExtractor:
    def __init__(
        self,
        mode: str,
        ner_model: str = "vihealthbert",
        retrieval_model: str = "cambridgeltl/SapBERT-UMLS-2020AB-all-lang-from-XLMR",
    ):
        """
    ...
    def _detect_lang(self, text: str) -> str:
        ...
        vi_chars = re.compile(
            r"[àáãạảăắằẵặẳâấầẫậẩđèéẽẹẻêếềễệểìíĩịỉòóõọỏôốồỗộổơớờỡợởùúũụủưứừữựửỳýỹỵỷ]",
            re.IGNORECASE,
        )
    ...
    def _get_ner_instance(self, lang="vi"):
        ...
        if (
            self._ner_instance is None
            or getattr(self._ner_instance, "mode", "") != ner_lang_mode
        ):
    ...
    def _get_mapped_db(self):
        if self._base_df_cache is None:
            ...
            try:
                ...
                df["macro_type"] = df.get(
                    "original_type", df.get("mapped_type", "Other")
                )
        ...
        if self._external_kg_cache is None:
            try:
                ...
                self._external_kg_cache = df_ekg[
                    df_ekg["labels"].isin(["Diagnosis", "Disease", "Phenotype"])
                ]
            except Exception as e:
    ...
    def _get_doc_classifier(self, lang="vi"):
        ...
        if lang == "en":
            if self._doc_classifier_en is None:
                ...
                self._doc_classifier_en = PLMICDModel(
                    num_labels=len(classes), model_name=model_name
                )
                ...
                if os.path.exists(en_model_path):
                    ...
                    for k in list(state_dict.keys()):
                        if (
                            k in current_state
                            and state_dict[k].shape != current_state[k].shape
                        ):
                            if len(state_dict[k].shape) == 1:
                                new_tensor = current_state[k].clone()
                                min_size = min(
                                    new_tensor.shape[0], state_dict[k].shape[0]
                                )
                                ...
                                min_dim1 = min(
                                    new_tensor.shape[1], state_dict[k].shape[1]
                                )
                                new_tensor[:min_dim0, :min_dim1] = state_dict[k][
                                    :min_dim0, :min_dim1
                                ]
                                state_dict[k] = new_tensor
            ...
            if self._doc_classifier_vi is None:
                ...
                self._doc_classifier_vi = PLMICDModel(
                    num_labels=len(classes), model_name="vihealthbert"
                )
    ...
    def _run_ner_retrieval(self, text: str, lang: str) -> List[Dict[str, Any]]:
        ...
        for idx, ent in enumerate(ner_results):
            ...
            codes = (
                self._get_cui_vocab_codes(cui)
                if cui and cui.startswith("C") and not cui.startswith("C_")
                else {}
            )
    ...
    def _run_doc_class(
        self, text: str, lang: str, threshold: float = 0.5, dl_model: str = "auto"
    ) -> List[Dict[str, Any]]:
            ...
            if prob >= threshold:
                ...
                if not ekg_df.empty:
                    ...
                    if not match.empty:
                        row = match.iloc[0]
                        cols_to_check = (
                            ["name", "medgemma_trans", "qwen_trans", "map_trans"]
                            if lang == "en"
                            else ["medgemma_trans", "qwen_trans", "map_trans", "name"]
                        )
